...
Class model_cursos extends model{
	...
	public function carrega_curso($id){		
		$db = new mysql();
		$exec = $db->executar("SELECT * FROM cursos WHERE id='$id' ");
		return $exec->fetch_object();
	}
    ...
    public function curso_conteudo($codigo){
	 ...
	 public function curso_conteudo_varios($codigo){
		$db = new mysql();
		$exec = $db->executar("SELECT * FROM conteudo_curso_topico WHERE id_produto in ( $codigo ) order by id asc");
		// echo'<pre>';print_r("SELECT * FROM conteudo_curso_topico WHERE id_produto in ( $codigo ) order by id asc");exit;
