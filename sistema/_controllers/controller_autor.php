...
class autor extends controller {
	...
	public function inicial(){
		...
		$autor = new model_autor();		
		// intancia 
		$dados['lista'] = $autor->lista();
		
		$this->view('autor', $dados);
	}
	...
	public function novo(){
		...
		$this->view('cadastro.autor.novo', $dados);
	}
	...
	public function alterar_autor(){
        ...
        // instancia
		$autor = new model_autor();	
        $dados['data'] = $autor->autor_by_id($codigo);

		$dados['link'] = DOMINIO.$this->_controller.'/alterar_autor/codigo/'.$codigo.'/aba/conteudo_curso';
		$this->view('autor.alterar', $dados);
	}

    public function alterar_autor_dados(){
		...
		$this->irpara(DOMINIO.$this->_controller.'/alterar_autor/codigo/'.$id.'/aba/dados');
	}
	...
	public function apagar_varios(){
		...
		foreach ($autor->lista() as $key => $value) {
			if($this->post('apagar_'.$value['id']) == $value['id']){
				$autor->apagar($value['id']);
			}
		}
		$this->irpara(DOMINIO.$this->_controller);
	}
