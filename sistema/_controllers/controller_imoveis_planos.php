...
class imoveis_planos extends controller {
	
	protected $_modulo_nome = "Planos de Divulgação de Imóveis";

	public function init(){
		$this->autenticacao();
		$this->nivel_acesso(134);
	}
	...
	public function inicial(){
		...
		$this->view('imoveis_planos', $dados);
	}
	...
	public function novo(){
		...
		$this->view('imoveis_planos.novo', $dados);
	}
	...
	public function novo_grv(){
		...
		$db = new mysql();
		$db->inserir("imoveis_planos", array(
			"codigo"=>"$codigo",
			"titulo"=>"$titulo"
		));
	...
	public function alterar(){
		...
		$this->view('imoveis_planos.alterar', $dados);
	}
	...
	public function alterar_grv(){
		...
		$db = new mysql();
		$db->alterar("imoveis_planos", array(
			"titulo"=>$titulo,
			"valor"=>$valor_tratado,
			"meses"=>$meses,
			"dias"=>$dias,
	...
	public function apagar_varios(){
		...
		$db = new mysql();
		$exec = $db->Executar("SELECT * FROM imoveis_planos ");
		while($data = $exec->fetch_object()){
			
			if($this->post('apagar_'.$data->id) == $data->codigo){ 			 
				
				$conexao = new mysql();
				$conexao->apagar("imoveis_planos", " codigo='$data->codigo' AND id!=1 ");
	...
	public function gratis(){
		...
		$db = new mysql();
		$exec = $db->Executar("SELECT * FROM imoveis_planos where id='1' ");
		$dados['data'] = $exec->fetch_object();
		
		$this->view('imoveis_planos.gratis', $dados);
	}
