...
class index extends controller {
	...
	public function init(){
		// $this->inicializacao();
		// print_r(DOMINIO.'/cursos');
		$this->irpara(DOMINIO.'pedidos');
	...
	public function inicial(){
		...
		$result = $conexao->query("SELECT distinct 
									...
									where  canal.status <= 3 order by canal.status asc;");
		$new = array();
	...
	public function canais(){
		...
		$result = $conexao->query("SELECT distinct 
									...
									inner join loja.canal ON canal.id_canal = produto_canal.id_canal 
									inner join (select max(id) id, codigo, imagem from loja.produto_imagem group by codigo) t1 on produto.codigo=t1.codigo
									where  canal.id_canal = '$id' order by produto.id desc ");
		...
		$avaliacao = $conexao->query("SELECT distinct 
									...
									inner join loja.canal ON canal.id_canal = produto_canal.id_canal 
									inner join (select max(id) id, codigo, imagem from loja.produto_imagem group by codigo) t1 on produto.codigo=t1.codigo
									where  canal.id_canal = '$id' order by produto.avaliacao desc ");
		...
		// echo '<pre>'; print_r($avaliacao_new);exit;
	...
	public function update_clients_vindi(){
		...
		public function vindi_flow(){
			...
			if($recorrencia == 1){
				...
				$bill = $this->pay_bill_vindi($id_client,$payment_met,1040228,$total_amount);
			}
		...
		public function vindi_add_subscription($id_client,$payment_met,$product_ir,$plan_id){
			...
			CURLOPT_HTTPHEADER => array(
					...
					'authorization: Basic T2dzSlpndmZPa0N2NmtfeGZ4VlJvT2hLMDE1ZHBoVDJ0WjNKZVMyWFExTTp1bmRlZmluZWQ=',
		...
		public function pay_bill_vindi($id, $payment,$product_id,$amount){
	...
	public function pedidos_detalhes(){
		...
		while($data = $nome->fetch_object()){
			...
			$dados['fisica_nascimento'] = $data->fisica_nascimento;
			$dados['cep'] 				= $data->cep;
