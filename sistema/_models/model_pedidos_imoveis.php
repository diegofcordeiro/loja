...
Class model_pedidos_imoveis extends model{
	...
	public function lista_incompletos(){
		...
		$db = new mysql();
		$exec = $db->executar("SELECT * FROM imoveis_pedidos where status='0' order by id desc"); 
		while($data = $exec->fetch_object()) { 
			$lista[$i]['id'] = $data->id;
	...
	public function lista_aguardando(){
		...
		$db = new mysql();
		$exec = $db->executar("SELECT * FROM imoveis_pedidos where status='1' order by id asc");
		$i = 0;
		while($data = $exec->fetch_object()) { 
	...
	public function lista_aprovados(){
		...
		$db = new mysql();
		$exec = $db->executar("SELECT * FROM imoveis_pedidos where status='2' order by id asc");
		$i = 0;
		while($data = $exec->fetch_object()) { 
	...
	public function carrega($codigo){
		$db = new mysql();
		$exec = $db->executar("SELECT * FROM imoveis_pedidos where codigo='$codigo' ");
