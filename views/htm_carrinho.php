...
<body>
	...
	foreach ($layout_lista as $key_layout => $value_layout) {
		...
		if ($value_layout['full'] != 1) {
			echo "<div class='container cont_meio' >";
		}
						...
						<!-- <div style="padding-top: 10px;"> 
							<div class="input-group">
								...
								<span class="input-group-btn">
									<button class="btn botao_padrao <?= $botao_css ?>" type="button" onClick="verifica_cupom()">Inserir Cupom</button>
								</span>
							</div>
						</div> -->
						...
						<?php if ($tipo_envio != 1) { ?>
							...
							<!-- <div style="margin-top:30px; margin-bottom:30px;"  >
								...
								<div class="input-group">
									...
									<span class="input-group-btn">
										<button class="btn botao_padrao <?= $botao_css ?>" type="button" onClick="verifica_cep()">Calcular Frete</button>
									</span>
								</div>
				...
				<div class="row">
					...
					<div class="col-sm-6">
						<div class="ajuste_botoes_carrinho_e">
							...
							<button type='button' class='botao_padrao <?= $botao_css ?>' onClick="window.location='<?= DOMINIO ?>';">Continuar Comprando</button>
					...
					<div class="col-sm-6">
						...
						<div class="ajuste_botoes_carrinho_d" id="div_bt_finalizar">
							...
							<button type="button" class="botao_padrao <?= $botao_css ?>" onClick="finaliza_pedido();">Finalizar Pedido</button>
	...
	<script>
			...
			$.post('<?= DOMINIO ?><?= $controller ?>/fechar_pedido', '', function(data) {
				...
				if (data) {
					...
					if (retorno.processo == 'ok') {
						...
						window.location = "<?= DOMINIO ?><?= $controller ?>/minhaconta";
					} else {
		...
		function cidades(estado, selecionado = null) {
			...
			$.post('<?= DOMINIO ?><?= $controller ?>/balcoes_cidades', {
				estado: estado,
		...
		function lista_balcoes(estado, cidade) {
			...
			$.post('<?= DOMINIO ?><?= $controller ?>/balcoes_lista_carrinho', {
				estado: estado,
				cidade: cidade,
				subtotal: '<?= $carrinho['subtotal'] ?>',
				selecionado: '<?= $data_pedido->frete_balcao ?>'
			}, function(data) {
				if (data) {
		...
		function atualiza_lista_produtos() {
			...
			$.post('<?= DOMINIO ?><?= $controller ?>/lista_carrinho', {
				token: '<?= time() ?>'
			}, function(data) {
				if (data) {
		...
		function remove_carrinho(id) {
			$.post('<?= DOMINIO ?><?= $controller ?>/remover_carrinho/id/' + id, {
				token: '<?= time() ?>'
			}, function(data) {
				if (data) {
					atualiza_lista_produtos();
		...
		function altera_quantidade(id) {
			...
			$.post('<?= DOMINIO ?><?= $controller ?>/quantidade', {
				token: '<?= time() ?>',
		...
		function atualiza_valores() {
			...
			$.post('<?= DOMINIO ?><?= $controller ?>/lista_valores', {
				token: '<?= time() ?>'
			}, function(data) {
				if (data) {
		...
		function verifica_cupom() {
			var cupom = $('#cupom').val();
			$.post('<?= DOMINIO ?><?= $controller ?>/carrinho_cupom', {
				token: '<?= time() ?>',
				cupom: cupom
			}, function(data) {
				if (data) {
		...
		function verifica_cep() {
			...
			if (cep) {
				...
				$.post('<?= DOMINIO ?><?= $controller ?>/carrinho_cep', {
					token: '<?= time() ?>',
					cep: cep
				}, function(data) {
					if (data) {
		...
		function seleciona_frete(id, valor) {
			$('#lista_valores').html("<div style='text-align:center;'><img src='<?= LAYOUT ?>img/loading.gif' style='border:0px; width:250px;' ></div>");
			$.post('<?= DOMINIO ?><?= $controller ?>/carrinho_frete/id/' + id + '/valor_subtotal/' + valor, {
				token: '<?= time() ?>'
			}, function(data) {
				if (data) {
					atualiza_valores();
		...
		function seleciona_pg(id) {
			...
			$.post('<?= DOMINIO ?><?= $controller ?>/carrinho_forma_pagamento/id/' + id, {
				token: '<?= time() ?>'
			}, function(data) {
				if (data) {
					atualiza_valores();
