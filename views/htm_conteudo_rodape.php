...
<?php if ($conteudo_sessao['data_rodape']->modelo == 1) { ?>
	...
	?>
		<div class="fot-fixd politica_cookies" style="background-color: <?= $primaria ?>; color:<?= $cores['21'] ?>; ">
			<div class="fot-fixd-msg"><?= $conteudo_sessao['data_rodape']->aviso_conteudo ?></div>
			<div class="fot-fixd-box clearfix"><?= $botao_aviso ?></div>
		</div>
	<?php
	}
	?>
...
<script type="text/javascript">
	function aceitar_cokies() {
		$.post('<?= DOMINIO ?>index/cookies_aceitar', {
			token: '<?= time() ?>'
		}, function(data) {
			if (data) {
